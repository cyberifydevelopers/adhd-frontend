...
export function TaskSessionDialogs() {
  ...
  const milestone = taskPauseStore((s) => s.checkpointMilestone);
  const closeExit = taskPauseStore((s) => s.closeExitConfirm);
  const resumeSession = taskPauseStore((s) => s.resumeSession);
  const dismissMilestone = taskPauseStore((s) => s.dismissCheckpointMilestone);
  ...
  return (
    <>
      ...
      >
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            <DialogTitle>{milestone?.title ?? "Checkpoint update"}</DialogTitle>
            <DialogDescription>{milestone?.description}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" variant="primary" className="w-full" onClick={() => dismissMilestone()}>
              Continue test
            </Button>
          </DialogFooter>
        </DialogContent>
